...
for (var candidate : jwtCandidates) {
    final String finalLocatedToken = candidate.token;
    final String finalTokenHeaderName = candidate.headerName;
    final String finalTokenHeaderValue = candidate.headerValue;
    final int finalJwtIndex = candidate.globalIndex;
    final int finalJwtIndexInHeader = candidate.indexInHeader;

    logging.logToOutput("");
    logging.logToOutput("========== TESTING JWT #" + finalJwtIndex + " ==========");
    logging.logToOutput("[*] Header: " + finalTokenHeaderName);
    logging.logToOutput("[*] Position in header: " + finalJwtIndexInHeader);

    var jwtParts = finalLocatedToken.split("\\.", -1);
    if (jwtParts.length < 2) {
        logging.logToOutput("[!] JWT #" + finalJwtIndex + " is not in compact JWT format.");
        continue;
    }
    ...
    if (decodedHeader == null || decodedPayload == null) {
        logging.logToOutput("[!] Unable to decode JWT #" + finalJwtIndex + " header/payload.");
        continue;
    }
    ...
    try {
        ...
        logging.logToOutput("[!] Decoded JWT #" + finalJwtIndex + " header/payload is not valid JSON.");
        continue;
    }
    ...
    java.util.function.Function<String, burp.api.montoya.http.message.requests.HttpRequest> withJwtReplaced = (newToken) -> {
        ...
        return baseReq.withUpdatedHeader(finalTokenHeaderName, newHeaderValue).withService(baseReq.httpService());
    };
    ...
    logging.logToOutput("[*] JWT #" + finalJwtIndex + " header: " + decodedHeader);
    ...
    java.util.function.BiConsumer<String, String> testToken = (label, tokenToSend) -> {
        try {
            ...
            if (mutatedReq.httpService() == null) {
                logging.logToOutput("[!] JWT #" + finalJwtIndex + " | No HttpService for test: " + label);
                return;
            }

            var result = api.http().sendRequest(mutatedReq);
            if (result.response() == null) {
                logging.logToOutput("NONE [JWT #" + finalJwtIndex + "] " + label + " | no response");
                return;
            }
            ...
            if (st < 400
                ...
                logging.logToOutput(st + " [HIT] JWT #" + finalJwtIndex + " | " + finalTokenHeaderName + " | " + label);
            } else if (verbose) {
                logging.logToOutput(st + " [DEBUG] JWT #" + finalJwtIndex + " | " + finalTokenHeaderName + " | " + label + " | verbose error exposed");
            } else {
                logging.logToOutput(st + " [OK] JWT #" + finalJwtIndex + " | " + finalTokenHeaderName + " | " + label);
            }
        } catch (Exception e) {
            logging.logToOutput("[!] Exception during JWT #" + finalJwtIndex + " test " + label + ": " + e.getMessage());
        }
    };
    ...
    logging.logToOutput("--- JWT #" + finalJwtIndex + " FINDINGS (" + jwtFindings.size() + ") ---");
    for (String f : jwtFindings) logging.logToOutput(f);

    logging.logToOutput("--- JWT #" + finalJwtIndex + " PASSED (" + jwtPassed.size() + ") ---");
    for (String p : jwtPassed) logging.logToOutput("[OK] " + p);

    logging.logToOutput("--- JWT #" + finalJwtIndex + " MANUAL REVIEW (" + jwtManual.size() + ") ---");
    for (String m : jwtManual) logging.logToOutput("[MANUAL] " + m);

    logging.logToOutput("========== END JWT #" + finalJwtIndex + " ==========");
}

logging.logToOutput("");
logging.logToOutput("========== END JWT / BEARER CHECKER ==========");
